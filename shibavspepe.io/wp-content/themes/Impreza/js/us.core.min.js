        ...
        setup: function(_, ns, handle) {
            ...
            if (!!self.addEventListener) {
                self.addEventListener(type, handle, {
                    passive: type.indexOf('touch') === 0 ? !ns.includes('noPreventDefault') : !0
                })
            }
        }
    }
...
$us.mixins.Events = {
    ...
    trigger: function(eventType, extraParameters) {
        ...
        var args = arguments,
            params = (args.length > 2 || !jQuery.isArray(extraParameters)) ? Array.prototype.slice.call(args, 1) : extraParameters;
        params.unshift(self);
...
! function($, undefined) {
    ...
    function USCanvas(options) {
        ...
        $us.$window.on('scroll', self._events.scroll.bind(self)).on('resize load', self._events.resize.bind(self)).on('scroll resize load', self._events.toggleClassIsSticky.bind(self));
...
if ($us.$body.hasClass('single-format-video')) {
    ...
    $us.collapsibleContent.prototype = {
        init: function(container) {
            ...
            self.$container.on('click', '.collapsible-content-more, .collapsible-content-less', self._events.toggleContent.bind(self));
        ...
        _events: {
            toggleContent: function(e) {
                ...
                e.stopPropagation();
                var self = this;
                self.$container.toggleClass('show_content', $(e.target).hasClass('collapsible-content-more')).trigger('toggleContent');
                $ush.timeout(function() {
            ...
            for (const chart in this.charts) {
                ...
                if (this.$container.closest($wrapper).length) {
                    if (this.$container.hasClass(this.charts.line.class)) {
                        $.fn.vcLineChart && this.$container.vcLineChart({
                            reload: !1
                        })
                    } else if (this.$container.hasClass(this.charts.round.class)) {
                        $.fn.vcRoundChart && this.$container.vcRoundChart({
                            reload: !1
                        })
                    }
                }
...
(function($) {
    ...
    $.fn.wDropdown = function() {
        return this.each(function() {
            ...
            $self.on('click', 'a[href$="#"]', function(e) {
                e.preventDefault()
...
! function($, undefined) {
    ...
    $us.WForm = function(container) {
        ...
        $('input[type=text],\
			...
			input[type=url],\
			input[type=password],\
			textarea', self.$form).each(function(_, input) {
            var $input = $(input),
    ...
    $(_document).on('focus blur', '.w-form-row-field:not(.for_date) input, .w-form-row-field textarea', function(e) {
        $(e.target).closest('.w-form-row').toggleClass('focused', e.type == 'focusin')
    })
}(jQuery);
! function($) {
    "use strict";
...
! function($, undefined) {
    ...
    $.extend($us.wGmaps.prototype, {
        ...
        _confirm: function() {
            ...
            self.$container.html($ush.base64Decode('' + $('script[type="text/template"]', self.$container).text())).removeAttr('data-cookie-name');
            self[self.isGmapLoaded() ? '_init' : '_initAftetGmapLoaded']()
        },
        _init: function() {
            var self = this;
            if (self.$container.is('[data-cookie-name]') || !self.isGmapLoaded()) {
                return
            }
            ...
            if (self.style != _null && self.style != {}) {
                self._mapInstance.map.setOptions({
                    styles: self.style
                })
            }
            var shouldRunGeoCode, matches = $ush.removeSpaces('' + self.options.address).match(_PATTERN_EXTRACT_COORDINATES_);
            if (matches) {
                ...
                $us._wGmapsGeocodes.add(self.uniqid, function(stopGeocodeTask) {
                    self._mapGeoCode(self.uniqid, self.options.address, function(latitude, longitude) {
            ...
            $.each(self.options.markers, function(i, _) {
                ...
                if (self.options.icon != _null || self.options.markers[i].marker_img != _null) {
                    ...
                    var size = new google.maps.Size($ush.parseInt(width), $ush.parseInt(height));
                    markerOptions.icon = {
                        url: url,
                        size: size,
                        scaledSize: size,
                ...
                if (self.options.markers[i] != _null) {
                    var matches = $ush.removeSpaces(self.options.markers[i].address).match(_PATTERN_EXTRACT_COORDINATES_);
                    if (matches) {
                        ...
                        $us._wGmapsGeocodes.add(markerGeocodeId, function(stopGeocodeTask) {
                            self._mapGeoCode(markerGeocodeId, self.options.markers[i].address, function(latitude, longitude) {
                                ...
                                var marker = self._mapInstance.addMarker(markerOptions);
                                if (self.options.markers[i].infowindow) {
                                    marker.infoWindow.open($ush.clone(self._mapInstance.map, {
                                        shouldFocus: !1
                                    }), marker)
                                }
                                if ($.isFunction(stopGeocodeTask)) {
        ...
        _initAftetGmapLoaded: function() {
            ...
            $script.attr('src', ('' + $script.data('src')).replace('&#038;', '&')).removeAttr('data-src')
        }
    });
    $.fn.wGmaps = function(options) {
        ...
        return this.each(function() {
            ...
            $us.getScript($us.templateDirectoryUri + '/common/js/vendor/gmaps.js', function() {
                self._wGmaps = new $us.wGmaps(self, $ush.clone(options))
            })
        })
    };
    $(function() {
        ...
        init: function(container, options) {
            ...
            if (this.$list.hasClass('owl-carousel')) {
                $us.getScript($us.templateDirectoryUri + '/common/js/vendor/owl.carousel.js', function() {
                    ...
                    if (owlCarousel && this.carouselSettings.aria_labels.prev && this.carouselSettings.aria_labels.next) {
...
(function($, undefined) {
    ...
    $.extend($us.WGridFilter.prototype, $us.mixins.Events, {
        init: function(container, options) {
            ...
            self.$container.on('click', '.w-filter-opener', self._events.filterOpener.bind(self)).on('click', '.w-filter-list-closer, .w-filter-list-panel > .w-btn', self._events.filterListCloser.bind(self));
            self.$filtersItem.on('change', 'input, select', self._events.changeFilter.bind(self)).on('click', '.w-filter-item-reset', self._events.resetItem.bind(self));
            $(options.gridPaginationSelector, self.$grid).on('click', '.page-numbers', self._events.loadPageNumber.bind(self));
            ...
            $('form.woocommerce-ordering', $us.$canvas).off('change', 'select.orderby').on('change', 'select.orderby', self._events.woocommerceOrdering.bind(self));
...
! function($, undefined) {
    ...
    function USHeader(settings) {
        ...
        $us.$window.on('scroll', $ush.debounce(self._events.scroll.bind(self), 1)).on('resize load', $ush.debounce(self._events.resize.bind(self), 1));
        ...
        self.$container.on('transitionend webkitTransitionEnd oTransitionEnd MSTransitionEnd', $ush.debounce_fn_1ms(function() {
    ...
    $.extend(USHeader.prototype, $us.mixins.Events, {
        ...
        _events: {
            ...
            showBtn: function(e) {
                ...
                e.stopPropagation();
                $us.$body.addClass('header-show').on(($.isMobile ? 'touchstart' : 'click'), self._events.hideMobileVerticalHeader.bind(self))
            },
...
! function($) {
    ...
    $us.Nav.prototype = {
        ...
        setFixedMobileMaxHeight: function() {
            this.$list.css('max-height', $us.canvas.winHeight - $us.header.getCurrentHeight(!0) + 'px')
        },
        resize: function() {
            if (this.$nav.length == 0) {
                return
    ...
    $.extend($us.wLmaps.prototype, {
        ...
        _beforeRender: function() {
            var self = this,
                ...
                matches = $ush.removeSpaces(address).match(_PATTERN_EXTRACT_COORDINATES_);
            if (matches) {
        ...
        _render: function() {
            ...
            self._renderMarkers();
            if ($us.usbPreview() || self.options.disableDragging && !$us.$html.hasClass('no-touch')) {
        ...
        _geocoder: function(text, markerOptions, popup) {
            ...
            $.getJSON(osmUrl, $.noop).done(function(json) {
                ...
                if (!markerOptions) {
                    ...
                    self.marker = L.marker([boundingBox[1], boundingBox[3]], markerOptions).addTo(self._mapInstance);
                    if (popup) {
        ...
        _redraw: function() {
            ...
            $ush.timeout(self._mapInstance.invalidateSize.bind(self._mapInstance, !0), 100)
        },
        _renderMarkers: function() {
            ...
            for (var i = 0; i < self.options.markers.length; i++) {
                ...
                if (i == 0) {
                    ...
                    var matches = $ush.removeSpaces(item.address).match(_PATTERN_EXTRACT_COORDINATES_);
                    if (matches) {
                        self.marker = L.marker([matches[1], matches[2]], markerOptions).addTo(self._mapInstance);
                        if (item.html) {
    ...
    $.fn.wLmaps = function(options) {
        ...
        return this.each(function() {
            ...
            $us.getScript($us.templateDirectoryUri + '/common/js/vendor/leaflet.js', function() {
                self._wLmaps = new $us.wLmaps(self, $ush.clone(options))
            })
        })
    };
    $(function() {
...
(function($, undefined) {
    ...
    $us.PageScroller.prototype = {
        init: function(container, options) {
            ...
            $us.$canvas.on('contentChange', $ush.debounce(this._events.resize, 5));
            var _resize = $ush.debounce(this._events.resize, 50);
            $us.$window.on('scroll', $ush.debounce(this._events.scroll, 5)).on('resize', function(_, stopExecute) {
                if (!stopExecute) {
                    _resize()
                }
            });
        ...
        _attachEvents: function() {
            ...
            if ($us.$window.width() > self.disableWidth && $us.mobileNavOpened <= 0 && (!$us.$html.hasClass('cloverlay_fixed'))) {
                _document.addEventListener('mousewheel', self._events.mouseWheelHandler, {
                    passive: !1
                });
                _document.addEventListener('DOMMouseScroll', self._events.mouseWheelHandler, {
                    passive: !1
                });
                _document.addEventListener('MozMousePixelScroll', self._events.mouseWheelHandler, {
                    passive: !1
                });
            ...
            if (this.footerReveal && section.area === 'footer' && key !== _undefined) {
                if (_window.innerWidth > parseInt($us.canvasOptions.columnsStackingWidth) - 1) {
                    if (this.sections[key - 1] !== _undefined && this.sections[key - 1].area === 'footer') {
                        ...
                        var rowIndex = (this.sections[this.lastContentSectionIndex + key] !== _undefined) ? this.lastContentSectionIndex + key : key - 1;
        ...
        scroll: function() {
            ...
            __debounce_fn_500ms(function() {
                ...
                for (var index in this.sections) {
                    ...
                    if (scrollTop >= $ush.parseInt(section.top - 1) && scrollTop < $ush.parseInt(section.bottom - 1) && section.area === 'content' && this.activeSection !== index) {
...
! function($) {
    ...
    $us.WPopup.prototype = {
        ...
        hideOnLinkClick: function(event) {
            ...
            if (place !== '#' && place.indexOf('#') === 0 && this.$wrap.find(place).length) {
                return
            }
            this.hide()
        },
    ...
    USScroll.prototype = {
        ...
        scrollTo: function(place, animate) {
            ...
            if (animate) {
                ...
                $us.$window.on('keydown mousewheel DOMMouseScroll touchstart', self._events.cancel)
            } else {
...
! function($, undefined) {
    ...
    $us.WTabs.prototype = {
        init: function(container, options) {
            ...
            $us.$window.on('resize', $ush.debounce(this._events.resize, 5)).on('hashchange', this._events.hashchange).on('wheel', $ush.debounce(this._events.wheel.bind(this), 5));
...
(function($, undefined) {
    ...
    $us.wVideo = function(container) {
        ...
        self.$videoTag = '';
        self.cookieName = self.$container.data('cookie-name');
        if (!self.cookieName && !self.$container.hasClass('with_overlay')) {
            return
        }
        self.data = {};
        if (self.$container.is('[onclick]')) {
